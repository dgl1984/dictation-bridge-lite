...
4. Download that workflow's add-on artifact and repeat the Notepad installation, finalized-online-phrase, responsive-offline-echo, selection-replacement, Tools-menu, and NVDA-restart tests. Confirm that add-on reload does not duplicate the submenu.
5. Create and push a tag such as `v0.3.0-beta.1`. The release workflow verifies that the tag begins with the manifest version, rebuilds both architectures, and attaches only the versioned `.nvda-addon` to the release.
